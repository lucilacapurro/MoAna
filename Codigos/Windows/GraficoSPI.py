...
def funcObtenerCSV(directorio_actual, caso):
    # Veo desde qué ventana se inicializó GraficoSPI
    if caso == "Busqueda":
    ...
    print("id_global", id_global)
    
    nombre_excel = 'Datos ' + id_global + '.csv'
    print("nombre_excel", nombre_excel)
    print(caso)
...
##############################################################################################
def funcGenerarGraficos(caso):

    # Obtiene el CSV del paciente y levanta los datos 
    directorio_actual = os.path.abspath(os.path.dirname(__file__))
    path, id_global = funcObtenerCSV(directorio_actual, caso)
    ...
    evento = df['evento']
    alarma = df['alarma']
    descripcion_alarma = df['descripcion_alarma']
    ...
    hayalarmas = df[df.alarma != "-"]
    haypromedio = df[df.spi_promedio != "-"]

    SPIEstados = plotly.graph_objs.Figure()
    PreBasal = plotly.graph_objs.Scatter(x=(pre_basal.tiempo)/60, y=pre_basal.spi, fill='none', mode='lines', line = {'color' : 'rgb(100, 138, 176)', 'shape': 'spline'}, name="Pre Basal", hovertemplate = "<br>SPI: %{y}<br>")
    Basal = plotly.graph_objs.Scatter(x=(basal.tiempo)/60, y=basal.spi, fill='none', marker = {'color' : 'rgb(190, 213, 242)'}, name="Basal", hovertemplate = "<br>SPI: %{y}<br>")
    Induccion = plotly.graph_objs.Scatter(x=(induccion.tiempo)/60, y=induccion.spi, fill='none', marker = {'color' : 'rgb(244, 202, 225)'}, name="Inducción", hovertemplate = "<br>SPI: %{y}<br>")
    Mantenimiento = plotly.graph_objs.Scatter(x=(mantenimiento.tiempo)/60, y=mantenimiento.spi, fill='none', marker = {'color' : 'rgb(179, 236, 225)'}, name="Mantenimiento", hovertemplate = "<br>SPI: %{y}<br>")
    Despertar = plotly.graph_objs.Scatter(x=(despertar.tiempo)/60, y=despertar.spi, fill='none', marker = {'color' : 'rgb(232, 203, 248)'}, name="Despertar", hovertemplate = "<br>SPI: %{y}<br>")
    Recuperacion = plotly.graph_objs.Scatter(x=(recuperacion.tiempo)/60, y=recuperacion.spi, fill='none', marker = {'color' : 'rgb(253, 205, 152)'}, name="Recuperación", hovertemplate = "<br>SPI: %{y}<br>")
    ...
    Eventos = plotly.graph_objs.Scatter(x=(hayeventos.tiempo)/60, y=hayeventos.spi_promedio, name="Eventos", mode='markers', line=dict(color='black'), text=hayeventos.evento, hovertemplate = "<br>%{text}")
    Alarmas = plotly.graph_objs.Scatter(x=(hayalarmas.tiempo)/60, y=hayalarmas.spi_promedio, name="Alarmas", mode='markers', marker=dict(symbol='triangle-up', size=10), line=dict(color='yellow'), text=hayalarmas.descripcion_alarma, hovertemplate = "<br>%{text}")
    ...
    inic_pre_basal = (pre_basal.tiempo.iloc[0])/60
    fin_pre_basal = (pre_basal.tiempo.iloc[-1])/60
    ...
    colors = ['rgb(100, 138, 176)', 'rgb(190, 213, 242)', 'rgb(244, 202, 225)', 'rgb(179, 236, 225)', 'rgb(232, 203, 248)', 'rgb(253, 205, 152)']
    shapes = []
    bgs = [[inic_pre_basal, fin_pre_basal], [inic_basal, fin_basal], [inic_induccion, fin_induccion], [inic_mantenimiento, fin_mantenimiento], [inic_despertar, fin_despertar], [inic_recuperacion, fin_recuperacion]]
    ...
    color_linea_spi = '#e0e0e0'

    color_franja_amarilla = '#fff977'
    color_franja_verde = '#87ec55'
    color_franja_roja = '#ff715f'

    SPIFranjas = plotly.graph_objs.Figure()
    PreBasal = plotly.graph_objs.Scatter(x=(pre_basal.tiempo)/60, y=pre_basal.spi, fill='none', marker = {'color' : color_linea_spi}, name="Pre Basal", hovertemplate = "<br>SPI: %{y}<br>")
    Basal = plotly.graph_objs.Scatter(x=(basal.tiempo)/60, y=basal.spi, fill='none', marker = {'color' : color_linea_spi}, name="Basal", hovertemplate = "<br>SPI: %{y}<br>")
    Induccion = plotly.graph_objs.Scatter(x=(induccion.tiempo)/60, y=induccion.spi, fill='none', marker = {'color' : color_linea_spi}, name="Inducción", hovertemplate = "<br>SPI: %{y}<br>")
    Mantenimiento = plotly.graph_objs.Scatter(x=(mantenimiento.tiempo)/60, y=mantenimiento.spi, fill='none', marker = {'color' : color_linea_spi}, name="Mantenimiento", hovertemplate = "<br>SPI: %{y}<br>")
    Despertar = plotly.graph_objs.Scatter(x=(despertar.tiempo)/60, y=despertar.spi, fill='none', marker = {'color' : color_linea_spi}, name="Despertar", hovertemplate = "<br>SPI: %{y}<br>")
    Recuperacion = plotly.graph_objs.Scatter(x=(recuperacion.tiempo)/60, y=recuperacion.spi, fill='none', marker = {'color' : color_linea_spi}, name="Recuperación", hovertemplate = "<br>SPI: %{y}<br>")
    SPI_promedio = plotly.graph_objs.Scatter(x=(haypromedio.tiempo)/60, y=haypromedio.spi_promedio, name="SPI Promedio", mode='lines', line = {'color' : 'rgb(0, 0, 0)'}, hovertemplate = "<br>Valor: %{y:.1f}<br>")
    Eventos = plotly.graph_objs.Scatter(x=(hayeventos.tiempo)/60, y=hayeventos.spi_promedio, name="Eventos", mode='markers', line=dict(color='black'), text=hayeventos.evento, hovertemplate = "%{text}")
    Alarmas = plotly.graph_objs.Scatter(x=(hayalarmas.tiempo)/60, y=hayalarmas.spi_promedio, name="Alarmas", mode='markers', marker=dict(symbol='triangle-up', size=10), line=dict(color='yellow'), text=hayalarmas.descripcion_alarma, hovertemplate = "<br>%{text}")
    ...
    colors = [color_franja_amarilla, color_franja_verde, color_franja_roja]
    shapes = []
    ...
    return SPIEstados, SPIFranjas, PorcentajesSPI, porcentajes